...
export default class Browser {
  ...
  constructor(key, jsonPath, overrideDescPath) {
    ...
    for (const entry of this._data) {
      const obj = {
        ...
        id: entry._id,
        description:
          entry.system?.description ?? entry.description ?? entry.text,
   ...
   */
  async render() {
    return await hbs.renderFromTemplate(this.paths.browser, {
      data: this._data,
      celestus,
    });
  }
  ...
  async description(id) {
    ...
    const path = this.overrideDescPath
      ? this.paths.description
      : `templates/descriptions/${item.type}-description.hbs`;
