/* global THREE, CANNON, $t, elem */

/**
 * @brief generates polyhedral dice with roll animation and result calculation
 * @author Anton Natarov aka Teal (original author)
 * @author Sarah Rosanna Busch (refactor, see changelog - Sarah)
 * @author Ethan Rosenberg (modifications, see changelog - Sarah)
 * @date 10 Aug 2023
 * @version 1.1
...
/**
 * CHANGELOG - Ethan
 * - Turned DICE into a function that returns an object for a program to use
...
/**
 * 
 * @param {Object?} options object containing configurable vars
 *                 .speed multiplier for how fast to render dice
 *                 .scalar scale to multiply dice size by
 *                 .material_options various material options for dice (specular, color, shininess)
 *                 .label_color color for numbers
 *                 .dice_color color for dice
 *                 .ambient_light_color
 *                 .spot_light_color
 *                 .desk_color canvas background
 *                 .desk_opacity
 *                 .use_shadows
 *                 .use_adaptive_timestep makes dice look better, set to false for better performance
 *                 .volume volume for dice sound effect
 * 
 * @returns DICE object to use for all rolling and display
 */
...
function DICE(options = {}) {
    ...
    const vars = {
        frame_rate: (options.speed ?? 1) / 60,
        scale: 100, //dice size
        scalar: options.scalar ?? 0.5,

        material_options: {
            specular: options.material_options?.specular ?? 0x172022,
            color: options.material_options?.color ?? 0xf0f0f0,
            shininess: options.material_options?.shininess ?? 10,
            shading: THREE.FlatShading,
        },
        label_color: options.label_color ?? '#aaaaaa', //numbers on dice
        dice_color: options.dice_color ?? '#202020',
        ambient_light_color: options.ambient_light_color ?? 0xf0f0f0,
        spot_light_color: options.h ?? 0xefefef,
        desk_color: options.desk_color ?? '#101010', //canvas background
        desk_opacity: options.desk_opacity ?? 0,
        use_shadows: options.use_shadows ?? true,
        use_adaptive_timestep: options.use_adaptive_timestep ?? true,
        volume: 0.5,
    }
    ...
    const CONSTS = {
        ...
        dice_face_range: {
            'd4': [1, 4], 'd6': [1, 6], 'd8': [1, 8], 'd9': [0, 9], 'd10': [0, 9],
    ...
    that.dice_box = function (container) {
        ...
        const ambientLight = new THREE.AmbientLight(vars.ambient_light_color);
    ...
    that.dice_box.prototype.reinit = function (container) {
        ...
        vars.scale = Math.sqrt(this.w * this.w + this.h * this.h) / 8 * vars.scalar;
    ...
    that.dice_box.prototype.start_throw = function (before_roll, after_roll) {
        ...
        throw_dices(box, vector, boost, dist, before_roll, after_roll, info);
    }
    ...
    function throw_dices(box, vector, boost, dist, before_roll, after_roll, info = {}) {
        const uat = vars.use_adaptive_timestep;

        vector.x /= dist; vector.y /= dist;
        const notation = that.parse_notation(box.diceToRoll);
        ...
        playSound(box.container, vars.volume);

        if (before_roll) {
            request_results = before_roll(notation, info);
        }
        ...
        function roll(request_results) {
            ...
            box.roll(vectors, request_results || notation.result, function (result) {
                ...
                if (after_roll) after_roll(notation, info);
    ...
    that.dice_box.prototype.generate_vectors = function (notation, vector, boost) {
        ...
        for (const i in notation.set) {
            ...
            const inertia = CONSTS.dice_inertia[notation.set[i]];
            const angle = {
    ...
    that.dice_box.prototype.create_dice = function (type, pos, velocity, angle, axis) {
        const dice = threeD_dice['create_' + type]();
    ...
    that.dice_box.prototype.__animate = function (threadid) {
        ...
        if (this.running == threadid) {
            ...
            })(this, threadid, vars.use_adaptive_timestep);
        }
    }
    ...
    function playSound(outerContainer, soundVolume) {
        ...
        audio.src = '/js/dice/assets/nc93322.mp3'; //todo: make this configurable
